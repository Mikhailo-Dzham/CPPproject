...
| :--- | :--- | :--- |
| **C (Union + switch)** | \~1.31914 с | Мінімальний оверхед, прямий доступ до пам'яті. |
| **C++ (Variant + visit)** | \~1.84697 с | Незначне уповільнення через перевірки типів у `std::visit`. |

**Аналіз:**
Реалізація на C виявилася трохи швидшою (на 0.52783с), оскільки `switch` компілюється у таблицю переходів (jump table), що є дуже ефективним. `std::variant` додає шар безпеки (перевірка індексу типу), що коштує кількох тактів процесора. Однак, різниця є мізерною для більшості прикладних задач, тому безпека C++ є більш пріоритетною.
